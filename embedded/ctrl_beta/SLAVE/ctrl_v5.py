...
def run():
    ...
    if pce.get_var('TRAIN_STATUS') != 1:
        ...
        # IF COLLECTING MVC
            ...
            print 'MVC'
            # Collect MVC data for speed and noise amplitude scaling
            ...
            if (pce.get_var('CLASS_ACTIVE') == 0) & (0 <= mode < numClasses):
                ...
                if mode == 0:
                    ...
                    mvc_temp = pce.get_var('MVC').to_np_array()
                    for i in range(0, numEMG):
                        mvc_temp[mode, i] = updateAverage(mvc_temp[mode, i], chan_mav[i], N_MVC[0, mode])
                    pce.set_var('MVC', mvc_temp)
...
def initialiseVariables():
    ...
    pce.set_var('N_MVC', np.zeros((1, numClasses), dtype=float, order='F'))
