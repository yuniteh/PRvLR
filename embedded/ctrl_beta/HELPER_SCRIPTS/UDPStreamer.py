...
def sendData():
    ...
    chan_mav = ';'.join(['%.5f' % num for num in pce.get_var('CHAN_MAV').to_np_array()])
    ...
    if streaming:
        ...
        if ctrl == 1:
            sendString = "C_OUT=" + str(class_out) + ",COLL=" + str(collect) + ",VEL=" + str(velocity) + ",N_T=" + npats + ",N_R=" + nreps + ",STR"
        elif ctrl == 2:
            sendString = "Y_EST=" + y_est + ",COLL=" + str(collect) + ",N_T=" + npats + ",N_R=" + nreps + ",STR"
        else:
            sendString = "COLL=" + str(collect) + ",N_T=" + mvcpats + ",N_R=" + mvcreps + ",STR"
    else:
        sendString = "nil"
    ...
    # If the message contains 'pce.set', a PCE message is being sent to be set on the embedded system.
        ...
        # If the data contains 'var', then this is a variable on the embedded system to be updated.
            ...
            try:
                ...
                if "DAQ_OUT_FNAME" in bits_str[3]:
